...
def floategral(f: function, a, b, n):
    return Calculus.floategral(f, a, b, n)
...
def critic_pofloats(f: function, a, b):
    return Calculus.FindCriticalPofloats(f, a, b)

def RiemannSum(f: function, a, b, n, type: Calculus.RienmannSumType):
    return Calculus.RienmannSum(f, a, b, n, type)

def definite_floategral(f: function, a, b, n):
    return Calculus.Definefloategral(f, a, b, n)

def exponentian(equation: str, value: float):
...
def binomial(base_value: float, exponent: float) -> float:
